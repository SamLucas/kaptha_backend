...
interface DRChemicalEquivalenceTerms {
  ...
  term_descritor_idterm_descritor: number;
}
...
export default async function FRChemicalEquivalenceTerms(
  data: DRChemicalEquivalenceTerms[]
): Promise<void> {
  const file: DRIndexPolifenols[] = JSON.parse(
    fs.readFileSync("./backup/indexPolifenols.json", "utf8")
  );
